...
// 2024-08-31 (Christoph Neuhauser): Set mouse cursor via sgl.
...
static void ImGui_ImplGlfw_UpdateMouseCursor()
{
    ...
    // 2024-08-31 (Christoph Neuhauser): Set mouse cursor via sgl.
    auto* sglWindow = static_cast<sgl::GlfwWindow*>(sgl::AppSettings::get()->getMainWindow());
    ...
    for (int n = 0; n < platform_io.Viewports.Size; n++)
    {
        ...
        if (window == sglWindow->getGlfwWindow())
        {
            if (imgui_cursor == ImGuiMouseCursor_None || io.MouseDrawCursor)
            {
                // Hide OS mouse cursor if imgui is drawing it or if it wants no cursor
                sglWindow->setShowCursor(false);
            }
            else
            {
                ...
                sglWindow->setCursorType(cursorType);
                sglWindow->setShowCursor(true);
            }
        }
        else
        {

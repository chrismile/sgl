...
public:
	...
	bool hasExtension(const char *fileString, const char *extension); // Check whether file has certain extension
	std::string filenameWithoutExtension(const std::string &filename);
	std::string getPureFilename(const std::string &path); // /home/user/Info.txt -> Info.txt
	std::string removeExtension(const std::string &path); // Info.txt -> Info
	...
	void rename(const std::string &filename, const std::string &newFilename); // Rename a file - you have to pass full paths twice!
	bool removeFile(const std::string &filename); // Delete file; returns success
	bool removeAll(const std::string &filename); // Delete file or directory inclusive all directories; returns success
...
private:
	int argc;
	char **argv;
	std::string titleName;	// Name of the application
	std::string execDir;	// Directory containing the application
	std::string configDir;	// Directory the app may write to, e.g. a folder in AppData (Windows) or .config (Linux)
	std::string userDir;	// Directory of the user, e.g. C:/Users/<Name> (Windows) or /home/<Name> (Linux)
	std::string sharedDir;	// Directoy available for all users on the system, e.g. /var/games (Linux) or just the configDir (Windows)
};

}

#endif /* SRC_UTILS_FILE_FILEUTILS_HPP_ */
